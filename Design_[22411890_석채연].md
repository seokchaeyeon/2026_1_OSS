<img width="1113" height="963" alt="Login" src="https://github.com/user-attachments/assets/e35db664-a45f-4c16-9129-d74ebc2e9a9c" />
...
<img width="1113" height="963" alt="Login" src="https://github.com/user-attachments/assets/755a9f69-1a43-4836-abc1-88a6d2b1d12b" />
...
sequenceDiagram
    ...
    Core->>Adapter: 1 : 금융 거래 발생 알림
    Adapter->>Adapter: 2 : 네트워크 연결 (coreIp, port 사용)
    Adapter->>Pipeline: 3 : receiverData()
    Pipeline->>Pipeline: 4 : rawData 속성에 원천 데이터 할당
    Pipeline-->>Adapter: 5 : 수신 완료 응답
```
...
sequenceDiagram
    ...
    Pipeline->>Pipeline: 1 : parseToTransaction()
    Pipeline->>Tx: 2 : Transaction 객체 생성 (속성 매핑)
    Tx-->>Pipeline: 3 : 객체 반환
    Pipeline->>Pipeline: 4 : insertData() (배열 적재)
    Pipeline->>Rule: 5 : 실시간 분석 큐(Queue)로 전달
```
...
sequenceDiagram
    ...
    Pipeline->>Rule: 1 : 단일 거래 분석 요청
    Rule->>Tx: 2 : getTransactionInfo()
    Tx-->>Rule: 3 : 거래 금액(amount) 및 상태 반환
    Rule->>Rule: 4 : detectHighAmount() 실행
    Rule->>Rule: 5 : activeRules 기준치 비교
    Rule-->>Pipeline: 6 : 1차 룰셋 분석 결과 반환
```
...
sequenceDiagram
    ...
    Rule->>Rule: 1 : detectSmurfing() 실행
    Rule->>Pipeline: 2 : searchPositionByTime()
    Pipeline->>Tx: 3 : 과거 거래 내역 속성 대조
    Tx-->>Pipeline: 4 : 매칭되는 거래 내역 반환
    Pipeline-->>Rule: 5 : 특정 시간대 분할 송금 배열 반환
    Rule->>Rule: 6 : 복합 패턴 검증 및 탐지 확정
```
...
sequenceDiagram
    ...
    Rule->>Rule: 1 : groupEvidence()
    Rule->>Log: 2 : createLog()
    Log->>Log: 3 : 위반 속성 매핑 (ruleName, detectedTime 등)
    Log->>Log: 4 : 증거 배열 구성 (evidenceTransactions)
    Log-->>Rule: 5 : 설명 가능한 화이트박스 로그 객체 반환
```
...
sequenceDiagram
    ...
    Rule->>Repo: 1 : 화이트박스 로그 저장 요청
    Repo->>Repo: 2 : dbUrl 속성 기반 커넥션 확인
    Repo->>DB: 3 : saveLog() 
    DB-->>Repo: 4 : 데이터베이스 적재 완료
    Repo-->>Rule: 5 : 영속성 보장 완료 응답
```
...
sequenceDiagram
    ...
    Rule->>Adapter: 1 : sendBlockCommand()
    Adapter-->>Rule: 2 : 즉시 차단 완료 응답
    Rule->>Dashboard: 3 : sendAlertPush()
    Dashboard->>Dashboard: 4 : updateStatistics() 실행
    Dashboard->>Dashboard: 5 : blockedCount 속성 증가
```
...
sequenceDiagram
    ...
    User->>Admin: 1 : 대시보드 내 전체 로그 조회 요청
    Admin->>Repo: 2 : fetchAllLogs()
    Repo-->>Admin: 3 : WhileBoxLog 리스트 반환
    User->>Admin: 4 : 특정 로그 상세 분석 요청
    Admin->>Repo: 5 : fetchLogById() (logId 전달)
    Repo-->>Admin: 6 : 상세 증거 내역(evidenceTransactions) 반환
    Admin-->>User: 7 : 상세 탐지 사유 화면 출력
```
...
sequenceDiagram
    ...
    User->>Admin: 1 : 특정 계좌 수동 동결 지시
    Admin->>Controller: 2 : executeManualLock()
    Controller->>Controller: 3 : targetAccount, authCode 검증
    Controller->>Adapter: 4 : sendBlockCommand()
    Adapter-->>Controller: 5 : 코어뱅킹 계좌 동결 완료
    Controller->>Tx: 6 : updateStatus()
    Tx-->>Controller: 7 : status 속성 변경 (Blocked)
```
...
sequenceDiagram
    ...
    User->>Admin: 1 : 고객 소명 완료 및 동결 해제 지시
    Admin->>Controller: 2 : executeUnlock()
    Controller->>Controller: 3 : targetAccount, authCode 검증
    Controller->>Adapter: 4 : sendUnblockCommand()
    Adapter-->>Controller: 5 : 코어뱅킹 계좌 동결 해제 완료
    Controller->>Tx: 6 : updateStatus()
    Tx-->>Controller: 7 : status 속성 변경 (Normal)
```
...
sequenceDiagram
    ...
    User->>Admin: 1 : 탐지 룰 임계치 수정 요청
    Admin->>Rule: 2 : updateRuleSettings()
    Rule->>Rule: 3 : activeRules 속성 재배포 및 메모리 갱신
    Rule-->>Admin: 4 : 룰셋 적용 완료
    User->>Admin: 5 : 업무 종료 및 시스템 로그아웃 요청
    Admin->>Dashboard: 6 : activeSessions 속성 감소 요청
    Admin->>Admin: 7 : logout() (계정 정보 파기)
```
---
