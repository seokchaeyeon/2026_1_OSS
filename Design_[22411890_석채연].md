...
```mermaid
stateDiagram-v2
    [*] --> 시스템대기(Standby) : 서버 구동 초기 상태
    
    시스템대기(Standby) --> 관리자_인증중(Authenticating) : Admin 계정 정보 입력 및 login()
    관리자_인증중(Authenticating) --> 시스템대기(Standby) : 자격 증명 실패
    
    관리자_인증중(Authenticating) --> 시스템_가동중(System_Active) : 인증 성공 및 Dashboard 세션 할당
    
    state 시스템_가동중(System_Active) {
        [*] --> 모니터링_대기(Monitoring_Idle)
        
        모니터링_대기(Monitoring_Idle) --> 데이터_파싱중(Parsing) : receiverData() 유입
        데이터_파싱중(Parsing) --> 실시간_분석중(Analyzing) : 객체 생성 후 RuleEngine 전달
        
        실시간_분석중(Analyzing) --> 정상_거래(Normal_Tx) : activeRules 룰셋 통과
        실시간_분석중(Analyzing) --> 이상거래_차단됨(Blocked_Tx) : 고액/스머핑 탐지 (sendBlockCommand)
        
        정상_거래(Normal_Tx) --> 모니터링_대기(Monitoring_Idle) : 통과 로그 저장 후 대기
        
        이상거래_차단됨(Blocked_Tx) --> 관리자_개입(Admin_Intervention) : sendAlertPush() 알림 발생
        
        관리자_개입(Admin_Intervention) --> 계좌_수동동결(Manual_Locked) : 2차 위험 판단 후 executeManualLock()
        관리자_개입(Admin_Intervention) --> 모니터링_대기(Monitoring_Idle) : 추가 조치 없이 단순 차단 유지
        
        계좌_수동동결(Manual_Locked) --> 수동동결_해제(Manual_Unlocked) : 고객 소명 및 executeUnlock()
        수동동결_해제(Manual_Unlocked) --> 모니터링_대기(Monitoring_Idle) : 계좌 상태 롤백 완료
    }
    
    시스템_가동중(System_Active) --> 룰셋_업데이트(Rule_Updating) : 관리자의 updateRuleSettings() 요청
    룰셋_업데이트(Rule_Updating) --> 시스템_가동중(System_Active) : 메모리 갱신 후 가동 복귀
    
    시스템_가동중(System_Active) --> [*] : logout() 및 세션 파기
```

위 그림은 화이트박스 FDS 시스템이 관리자의 로그인부터 시작하여 실시간 트랜잭션을 처리하고 최종적으로 로그아웃하기까지 겪는 통합 시스템 상태 전이(System State Transition)과정을 나타낸다.

시스템이 최초 구동된 시스템대기(Standby) 상태에서 관리자가 login()을 요청하면 관리자_인증중(Authenticating) 상태로 진입한다. 검증을 통과하면 모니터링 세션이 할당되며 전체 시스템이 시스템_가동중(System_Active)이라는 복합 상태(Composite State)로 전이된다.

가동 중인 시스템의 내부 로직은 모니터링_대기(Monitoring_Idle) 상태를 기본으로 유지한다. 코어뱅킹으로부터 데이터가 유입되면 데이터_파싱중(Parsing) 상태를 거쳐 RuleEngine이 가동되는 실시간_분석중(Analyzing) 상태로 넘어간다. 분석 결과에 따라 안전한 거래는 정상_거래(Normal_Tx) 상태를 거쳐 대기 상태로 복귀하지만 룰셋을 위반한 거래는 즉시 이상거래_차단됨(Blocked_Tx) 상태가 되며 관리자에게 푸시 알림을 보내 관리자_개입(Admin_Intervention) 상태를 유도한다.

이때 관리자의 판단에 따라 계좌 전체를 물리적으로 잠그는 계좌_수동동결(Manual_Locked) 상태로 심화될 수 있으며 향후 고객 소명 절차를 통해 수동동결_해제(Manual_Unlocked) 상태를 거쳐 다시 정상화될 수 있다.

가동 중인 상태에서 새로운 사기 패턴이 발견될 경우 관리자는 룰셋_업데이트(Rule_Updating) 상태로 진입하여 탐지 임계치를 실시간으로 갱신할 수 있다. 모든 업무가 종료되어 관리자가 logout()을 실행하면 시스템은 점유하던 리소스를 반환하고 최종 종료([*]) 상태에 도달하며 생명주기를 마감한다.
...
**5.1 H/W platform requirements**
  ...
  * (1) cpu: Intel Core i5/AMD Ryzen5 이상
  * (2) RAM: 8GB 이상
  * (3) Storage: SSD 50GB 이상의 여유공간

**5.2 S/W platform requirements

  * (1) OS: Microsoft Windows 10/11 / masOS
...
* **Core Banking (코어뱅킹):** 이체, 결제 등 은행의 핵심 원장 업무를 처리하는 중앙 시스템. 본 FDS 시스템은 어댑터(Adapter)를 통해 이 망으로부터 실시간 거래 데이터를 수신하고, 즉각적인 계좌 제어 명령(Block/Unblock)을 내림.
* **Transaction (트랜잭션):** 코어뱅킹 망에서 유입되는 개별 금융 거래를 의미하며, 시스템 내부에서 승인, 대기, 차단 등의 상태(Status)를 가지며 처리되는 가장 기본적인 데이터 추상화 객체.
* **Rule Engine (룰 엔진):** 메모리에 상주하는 탐지 규칙(activeRules)을 바탕으로 정렬된 트랜잭션 파이프라인을 감시하며, 고액 이체나 스머핑 등의 이상거래 여부를 실시간으로 판별하는 시스템의 심장부.
...
* **Manual Lock (수동 동결):** 시스템의 룰 기반 자동 차단과 별개로, 관리자가 대시보드에서 화이트박스 로그를 검토한 후 추가적인 위험을 인지하여 특정 계좌의 출금 상태를 즉각적으로 강제 정지시키는 보안 제어 로직.
...
* **Use Case Specification (유스케이스 명세서):** Use case description 표 양식을 지칭하는 정식 용어. 시스템의 특정 기능을 수행하기 위해 액터(Actor)와 시스템 간에 발생하는 상호작용의 순서를 정상 흐름(Main Success Scenario)과 예외 흐름(Extension Scenarios)으로 명확하고 상세하게 기술한 문서.
* **ACK (Acknowledgement):** 네트워크 통신에서 수신 측이 송신 측에게 데이터를 정상적으로 수신했음을 알리는 긍정 응답 신호. 본 시스템에서는 `DataPipeline`이 코어뱅킹 망으로부터 트랜잭션 데이터를 손실 없이 수신 완료했을 때 반환하는 확인 코드로 사용됨.
* **par (Parallel):** UML 시퀀스 다이어그램에서 여러 프로세스나 메시지가 동시에(병렬로) 실행됨을 나타내는 복합 프래그먼트(Combined Fragment). 본 설계에서는 이상거래 탐지 직후 DB 적재, 코어뱅킹 원장 차단, 관리자 대시보드 알림이 지연 없이 동시에 처리됨을 명시하기 위해 사용.
---

## 7. References
* [1]https://github.com/YashGames2007/Fraud-Detection-System-Backend
* [2]자바 네트워크 프로그래밍, 개정판(엘리어트 헤럴드)
