...
```Mermaid
stateDiagram-v2
    [*] --> 거래요청됨(Requested) : 사용자 거래 시도
    
    거래요청됨(Requested) --> 기본검증완료(Validated) : 1차 계좌/잔액 검증 통과
    거래요청됨(Requested) --> 거래거절(Rejected) : 잔액 부족 및 정보 오류
    
    기본검증완료(Validated) --> 분석중(Analyzing) : FDS 큐(Queue) 적재 및 분석 시작
    
    분석중(Analyzing) --> 위험도산출완료(Scored) : 룰 기반 스코어링 완료
    
    위험도산출완료(Scored) --> 거래차단(Blocked) : 고위험 판정 (임계치 초과)
    위험도산출완료(Scored) --> 추가인증요구(Pending) : 중위험 판정 (의심 거래)
    위험도산출완료(Scored) --> 거래승인(Approved) : 저위험 판정 (정상 거래)
    
    추가인증요구(Pending) --> 거래승인(Approved) : ARS/MFA 인증 성공
    추가인증요구(Pending) --> 거래차단(Blocked) : 인증 실패 또는 시간 초과
    
    거래차단(Blocked) --> 수동동결(Locked) : 관리자 개입 시 계좌 잠금
    수동동결(Locked) --> 거래거절(Rejected) : 소명 실패 시 영구 거절
    수동동결(Locked) --> 거래승인(Approved) : 고객 정상 거래 소명 완료
    
    거래승인(Approved) --> [*] : 코어뱅킹 처리 및 로그 저장
    거래거절(Rejected) --> [*] : 오류 안내 및 로그 저장
```
---
